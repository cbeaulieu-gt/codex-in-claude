"""Windows-shaped gitdir-pointer translation for WSL2 (fork issue #4).
...
that literally and cannot resolve it. Covers `_core.wslpath`:
  ...
  - `git_dir_override(cwd)` -- the `_base_git_env` chokepoint (plan §3.2):
    `{}` in every ordinary case (the containment property, plan §3.3);
    walks up from `cwd` to the first directory carrying a `.git` marker
    (the resolved §3.5 sub-directory design, walk-up with a depth/
    filesystem-boundary ceiling) and returns
    `{"GIT_DIR": ..., "GIT_WORK_TREE": ...}` only when that marker is a
    Windows-shaped linked-worktree pointer.

Per `AGENTS.md § Testing` ("a new parameter is new API surface"), every case
in the table at plan §5.2 is covered, plus the walk-up/ceiling behavior
resolved for §3.5.
"""
...
from codex_in_claude._core import wslpath
...
def test_normalize_unc_path_returned_unchanged():
    # UNC paths are explicitly out of scope (plan §5.2) -- must not half-translate.
...
def test_unc_path_gitdir_pointer_returns_none(tmp_path):
    # UNC values are out of scope -- must not half-translate (plan §5.2).
...
#
# Resolved design (plan §3.5, option (b)): walk up from `cwd` to the first
# directory containing a `.git` file/dir (with a depth/filesystem-boundary
# ceiling), setting GIT_WORK_TREE to *that* directory -- never to `cwd`
# itself, which would be actively wrong (git stops discovery once GIT_DIR is
# set).
...
def test_git_dir_override_returns_empty_when_no_dot_git_within_ceiling(tmp_path):
    ...
    # (a depth/filesystem-boundary ceiling per §3.5), never unbounded. The exact
    # ceiling value is an implementation choice; this asserts only the observable
